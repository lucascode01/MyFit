...
export function VideoCard({ video, onClick, onEdit, onDelete }: VideoCardProps) {
  ...
  return (
    <div className="card text-left w-full overflow-hidden rounded-xl relative group">
      <button
        type="button"
        onClick={onClick}
        className="w-full text-left hover:border-brand-orange/50 active:border-brand-orange/50 transition-colors touch-manipulation"
      >
        ...
        <div className="absolute top-2 right-2 flex gap-1 z-10" onClick={(e) => e.stopPropagation()}>
          {onEdit && (
            <button
              ...
              className="min-h-[36px] min-w-[36px] rounded-lg bg-black/50 hover:bg-black/70 text-white p-2"
              title="Editar"
          ...
          {onDelete && (
            <button
              ...
              className="min-h-[36px] rounded-lg bg-red-900/60 hover:bg-red-800/80 text-white px-2.5 py-2 flex items-center gap-1.5 text-sm font-medium"
              title="Excluir vídeo"
